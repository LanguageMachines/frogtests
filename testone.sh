...
#      \rm -f $file.err
      ...
      diff -w -B --ignore-blank-lines --ignore-matching-lines=".?*-annotation .?*" --ignore-matching-lines=".*generator=.*" --ignore-matching-lines=".*begindatetime=.*" --ignore-matching-lines=".*version=.*" --ignore-matching-lines="\b*" $file.out $file.ok >& $file.diff
      err=$?
      if [ $err -ne 0 ]
      then
	  for vari in {1..10}
	  do
	      ...
	      then
		  diff -w -B --ignore-blank-lines --ignore-matching-lines=".?*-annotation .?*" --ignore-matching-lines=".*generator=.*"  --ignore-matching-lines=".*version=.*" --ignore-matching-lines=".*begindatetime=.*" --ignore-matching-lines="\b*" $file.out $file.$vari.ok >& $file.diff
		  if [ $? -eq 0 ]
		  then
		      err=0
		      break
